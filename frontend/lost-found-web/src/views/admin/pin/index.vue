...
import { auditPinApi, getPinPageApi } from '@/api/pin'
import { PinRequestStatVO } from '@/types/pin'
import CommonTable from '@/components/common/CommonTable.vue'

const list = ref<PinRequestStatVO[]>([])
...
const audit = async (id: number, status: number) => {
  await auditPinApi(id, { status, remark: '' })
  await load()
}

const toDetail = (id: number) => {
  location.href = `/admin/pin/${id}`
}
</script>
